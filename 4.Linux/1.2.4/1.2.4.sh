...
function main {
    if ! $(check_dirs $2 $3); then
        echo "Source and destination directories should not be sibling" >&2
        exit 1
    fi
    ...
    if [ $s_dir_size -le $d_dir_av ]; then
        ...
        echo -n "Do you want to continue? (C | Y - continue, N | A - exit): "
        read answer 
...
case "$answer" in
    1 ) 
        ...
        ;;
    2 ) 
        echo -n "Enter the maximum number of copies: "
        read n_copy 
...
n_error=`wc -l $LOGFILE | awk '{ print $1 }'`

if [ $n_error -gt 0 ]; then
    echo -e "\e[1;31mWarning: $n_error error(s) occurred!\e[0m"
fi
